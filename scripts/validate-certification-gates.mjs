...
async function readJsonAt(file) {
  return JSON.parse(await readFile(file, 'utf8'));
}
async function readJson(name) {
  return readJsonAt(path.join(dataDir, name));
}
...
async function sourceHealthFiles() {
  ...
  try {
    ...
    for (const entry of entries) if (entry.isFile() && entry.name.endsWith('.json')) files.push(path.join(historyDir, entry.name));
  } catch {}
  return files;
}
...
const policy = await readJsonAt(path.join(root, 'config', 'execution-policy.json'));
...
check('data-quality', dataQuality >= 98 || (sourceGate === 'GREEN' && criticalFailures.length === 0), `committee data quality=${dataQuality}; target >=98 unless GREEN/no critical gaps`);
...
check('paper-policy-mode', policy.mode === 'PAPER_ONLY', `execution mode=${policy.mode}`);
check('paper-policy-broker-lock', policy.brokerConnectionAllowed === false, `brokerConnectionAllowed=${policy.brokerConnectionAllowed}`);
check('paper-policy-live-order-lock', policy.liveOrdersAllowed === false, `liveOrdersAllowed=${policy.liveOrdersAllowed}`);
check('paper-policy-real-capital-zero', Number(policy.maxRealCapitalEuro) === 0, `maxRealCapitalEuro=${policy.maxRealCapitalEuro}`);
check('paper-policy-human-reconfiguration', policy.requiresExplicitHumanReconfiguration === true, `requiresExplicitHumanReconfiguration=${policy.requiresExplicitHumanReconfiguration}`);
