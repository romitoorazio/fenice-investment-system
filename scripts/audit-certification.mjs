...
if (Number.isFinite(governance?.diagnostics?.intelligenceConfidence) && governance.diagnostics.intelligenceConfidence < 70) blockers.push(`intelligence confidence is ${governance.diagnostics.intelligenceConfidence}/100, expected >=70`);
if (Number.isFinite(governance?.diagnostics?.operationalSourceSharePercent) && governance.diagnostics.operationalSourceSharePercent < 80) blockers.push(`operational source share is ${governance.diagnostics.operationalSourceSharePercent}%, expected >=80%`);
if (sourceHealth.qualityScore < 100) warnings.push(`source quality not perfect: ${sourceHealth.qualityScore}/100`);

const report = { generatedAt: new Date().toISOString(), status: blockers.length ? "NOT_READY" : "READY", blockers, warnings, metrics: { sourceGate: sourceHealth.gate, sourceQuality: sourceHealth.qualityScore, criticalSourceFailures: sourceHealth.critical?.failures || [], ledgerRecords: ledger.records?.length || 0, markedLedgerRecords: marked.length, checkpointedLedgerRecords: checkpointed.length, universeAssets: universeAssets.length, assetClasses: assetClasses.size, terminalAssets: terminal.assets?.length || 0, intelligenceConfidence: governance?.diagnostics?.intelligenceConfidence ?? null, operationalSourceSharePercent: governance?.diagnostics?.operationalSourceSharePercent ?? null, liveTradingBlocked: g.blockAutonomousTrading === true } };
