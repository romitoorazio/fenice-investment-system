...
import { matchCatalysts } from './lib/catalyst-matcher.mjs';
...
function catalystFor(asset, discoveries) {
  const matches = matchCatalysts(asset, discoveries);
...
function buildCandidate(asset, fundamental, dcf, discoveries, dataQuality) {
  ...
  const decision = decisionFor({ score: committeeScore, confidence: sourceConfidence, risk, valuation, terminalDecision: asset.decision, dcf });
  ...
  return {
    ...
    maxWeightPercent,
    scorecard: { fundamental: fundamentalScore, quality: qualityScore, valuation, technical: technicalScore, riskAdjusted, catalysts: catalyst.score, dataConfidence: sourceConfidence },
    valuation: {
    ...
    catalyst: { score: catalyst.score, matchedEvents: catalyst.count, recentEvents: catalyst.recentCount, evidence: catalyst.evidence },
    entryPlan: entry,
    bullCase,
    bearCase,
    invalidation,
...
const dataQuality = Math.round(clamp(Number.isFinite(Number(sourceHealth.qualityScore)) ? Number(terminal.dataQuality || 0) * 0.65 + Number(sourceHealth.qualityScore) * 0.35 : Number(terminal.dataQuality || 0)));

const candidates = (terminal.assets || [])
  .map((asset) => buildCandidate(asset, fundamentalBySymbol.get(String(asset.symbol).toUpperCase()), dcfBySymbol.get(String(asset.symbol).toUpperCase()), snapshot.discoveries || [], dataQuality))
...
const report = {
  ...
  capitalEuro: CAPITAL,
  goal: { targetEuro: GOAL, horizonYears: YEARS, requiredCagrPercent: round(REQUIRED_CAGR, 2), warning: 'Target ambizioso, non garantito. Non aumenta automaticamente la tolleranza al rischio.' },
...
console.log(`Fenice Investment Committee v2: ${candidates.length} candidati, BUY ${buyCandidates.length}, gate ${executionGate}, qualità ${dataQuality}.`);
