  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "user-agent": "FeniceInvestmentSystem/3.6 institutional-source-validation",
      },
      redirect: "follow",
    });
...
async function collectCftc(snapshot, health) {
  ...
  const started = Date.now();
  try {
    const url = "https://publicreporting.cftc.gov/resource/gpe5-46if.json?$limit=25&$order=report_date_as_yyyy_mm_dd%20DESC";
    const { data, latencyMs } = await requestJson(url);
    const rows = Array.isArray(data) ? data : [];
    if (!rows.length) throw new Error("CFTC TFF returned no rows");
    const latestDate = rows[0]?.report_date_as_yyyy_mm_dd || rows[0]?.report_date;
    const openInterest = rows.reduce((sum, row) => sum + (finiteNumber(row?.open_interest_all) ?? 0), 0);

    addDiscovery(snapshot, {
      id: `cftc-tff-${String(latestDate || "latest").slice(0, 10)}`,
      name: "CFTC Traders in Financial Futures",
      category: "POSITIONING",
      signal: `Campione TFF acquisito: ${rows.length} contratti recenti${openInterest > 0 ? `, open interest campione ${Math.round(openInterest).toLocaleString("en-US")}` : ""}. Contesto di posizionamento, non segnale standalone.`,
      score: 60,
      risk: 45,
      date: latestDate,
    ...
    upsertProvider(snapshot, {
      ...
      name: "CFTC Traders in Financial Futures",
      state: "operativo",
      coverage: ["futures finanziari", "open interest", "posizionamento operatori"],
      detail: `${rows.length} righe TFF recenti acquisite dal Public Reporting Environment.`,
      lastSuccessAt: new Date().toISOString(),
    });
    pushHealth(health, { id, status: "healthy", records: rows.length, latencyMs, checkedAt: new Date().toISOString() });
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    upsertProvider(snapshot, { id, name: "CFTC Traders in Financial Futures", state: "errore", coverage: ["futures finanziari", "posizionamento"], detail });
    pushHealth(health, { id, status: "failed", records: 0, latencyMs: Date.now() - started, checkedAt: new Date().toISOString(), error: detail });
  }
}

async function collectFinra(snapshot, health) {
  const id = "finra-fixed-income";
  const started = Date.now();
  try {
    ...
    const detail = error instanceof Error ? error.message : String(error);
    upsertProvider(snapshot, { id, name: "FINRA Fixed Income API", state: "errore", coverage: ["Treasury TRACE", "fixed income"], detail });
    pushHealth(health, { id, status: "failed", records: 0, latencyMs: Date.now() - started, checkedAt: new Date().toISOString(), error: detail });
  }
}
