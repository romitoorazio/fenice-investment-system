...
for (const required of ["inviare ordini", "collegarsi a broker", "usare leva automaticamente"]) if (!prohibited.has(required)) fail(`missing prohibited action: ${required}`);
...
if (String(snapshot?.reliability?.institutionalSourceGate || "").toLowerCase() !== expectedGate) fail(`snapshot institutional gate does not match authoritative source health (${snapshot?.reliability?.institutionalSourceGate || "missing"} vs ${expectedGate})`);
...
for (const record of ledger.records) {
  ...
  if (!Number.isFinite(record.committeeScore) || !Number.isFinite(record.confidence) || !Number.isFinite(record.riskScore)) fail(`ledger record ${record.id} has incomplete scoring`);
  if ((record.proposedFirstTrancheEuro || 0) > 0 && record.executionGate !== "VAI") fail(`record ${record.id} proposes capital while execution gate is not VAI`);
}
