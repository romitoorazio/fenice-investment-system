...
check(Array.isArray(universe?.assets || universe?.instruments || universe) && (universe?.assets || universe?.instruments || universe).length >= 15, 'Global universe is missing or too small.');
...
// Paper trail must exist and be non-empty.
const ledgerEntries = Array.isArray(ledger?.entries) ? ledger.entries : Array.isArray(ledger) ? ledger : [];
check(ledgerEntries.length > 0, 'Decision ledger has no historical entries.');
...
// Execution gate may describe readiness for manual review, but never autonomous execution.
...
console.log(`Sources: ${sourceHealth.gate}; data quality: ${committee.dataQuality}/100; scanner assets: ${assets.length}; asset classes: ${assetClasses.size}; sectors: ${sectors.size}; ledger entries: ${ledgerEntries.length}; committee history: ${committeeHistoryCount}.`);
