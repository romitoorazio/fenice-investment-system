...
function sanitizeEndpoint(source, endpoint) {
  ...
  safe = safe.replace(/([?&](?:api[_-]?key|apikey|token|access[_-]?token|key)=)[^&#]+/gi, "$1[REDACTED]");
  return safe;
}
...
async function probe(source) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      if (result.ok) {
        return {
          ...
          regions: source.regions, endpointUsed: sanitizeEndpoint(source, endpoint), attempts, bytes: result.bytes, contentType: result.contentType,
        };
      }
  ...
  return {
    ...
    regions: source.regions, endpointUsed: sanitizeEndpoint(source, best?.endpoint ?? null), attempts,
