...
if (unsafe.length) {
  throw new Error(`Generated source-health contains unredacted credential material for: ${unsafe.join(", ")}`);
}

const serialized = JSON.stringify(health);
for (const forbidden of ["ALPHA_VANTAGE_API_KEY", "FRED_API_KEY", "EIA_API_KEY", "COINGECKO_API_KEY"]) {
  if (serialized.includes(`\"${forbidden}\":`)) {
    throw new Error(`Generated source-health must not serialize environment secret ${forbidden}`);
  }
}
