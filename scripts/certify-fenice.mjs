...
// Fail-closed execution safety. Certification must never unlock a broker.
pass(quality?.policy?.autonomousTrading === false,
  "SAFETY: autonomousTrading must remain false.");
pass(governance?.guardrails?.blockAutonomousTrading === true,
  "SAFETY: governance must hard-block autonomous trading.");
pass(governance?.guardrails?.requireHumanConfirmation === true,
  "SAFETY: actionable decisions must require human confirmation.");
...
for (const decision of committee?.topDecisions ?? []) {
  pass(Number.isFinite(decision?.riskScore) && decision.riskScore >= 0 && decision.riskScore <= 100,
    `RISK: invalid riskScore for ${decision?.symbol ?? "unknown"}.`);
  pass(Number.isFinite(decision?.maxWeightPercent) && decision.maxWeightPercent > 0 && decision.maxWeightPercent <= 15,
    `RISK: missing/unsafe maxWeightPercent for ${decision?.symbol ?? "unknown"}.`);
  pass(Array.isArray(decision?.invalidation) && decision.invalidation.length > 0,
    `RISK: missing invalidation conditions for ${decision?.symbol ?? "unknown"}.`);
  if (quality?.policy?.autonomousTrading === false) {
    pass(decision?.entryPlan?.orderMode === "NESSUN ORDINE" && Number(decision?.entryPlan?.firstTrancheEuro ?? 0) === 0,
      `SAFETY: ${decision?.symbol ?? "unknown"} exposes an executable entry while live trading is locked.`);
  }
}

// Persisted source-health artifacts must never expose credentials.
...
// Production data gates: no critical source failure and broad intelligence quality.
pass(sourceHealth?.gate === "GREEN",
  `DATA: global source gate is ${sourceHealth?.gate ?? "missing"}, expected GREEN.`);
pass((sourceHealth?.critical?.failures ?? []).length === 0,
  `DATA: critical source failures remain: ${(sourceHealth?.critical?.failures ?? []).join(", ") || "unknown"}.`);
pass(Number(sourceHealth?.critical?.ready ?? 0) === Number(sourceHealth?.critical?.total ?? -1),
  `DATA: critical source readiness is ${sourceHealth?.critical?.ready ?? "?"}/${sourceHealth?.critical?.total ?? "?"}.`);
pass(committee?.sourceGate === "GREEN",
  `DATA: committee sourceGate is ${committee?.sourceGate ?? "missing"}, expected GREEN.`);
...
pass(erroredSources.length === 0,
  `DATA: errored intelligence sources remain: ${erroredSources.map(source => source.name ?? source.id).join(", ") || "none"}.`);

// Scanner breadth and structured risk coverage.
...
pass(assetClasses.size >= 3,
  `SCANNER: intelligence quality covers only ${assetClasses.size} asset classes, expected >= 3.`);

// Paper process must be measurable and historically marked-to-market.
...
pass(ledgerRecords.some(record => Number.isFinite(record?.markToMarketPercent)),
  "PAPER: no marked-to-market decision records found.");
pass(ledgerRecords.some(record => record?.checkpoints && Object.keys(record.checkpoints).length > 0),
...
if (Number.isFinite(committee?.dataQuality) && Number.isFinite(quality?.intelligenceConfidence)) {
  ...
  pass(gap <= 20,
    `CONSISTENCY: committee dataQuality (${committee.dataQuality}) and intelligenceConfidence (${quality.intelligenceConfidence}) differ by ${gap} points.`);
}

const report = {
  generatedAt: new Date().toISOString(),
  certified: failures.length === 0,
  liveTradingLocked: quality?.policy?.autonomousTrading === false && governance?.guardrails?.blockAutonomousTrading === true,
  failures,
  warnings,
};
