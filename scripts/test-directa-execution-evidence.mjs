...
const snapshot = {
  ...
  quotes: [
    { ticker: "MSFT", observedAt: "17:30:02", priceObservedAt: "17:30:02", bookObservedAt: "17:30:02", metadataObservedAt: "17:29:59", lastPrice: 500, lastQuantity: 10, dayLow: 495, dayHigh: 505, bidPrice: 499.9, bidQuantity: 100, askPrice: 500.1, askQuantity: 120, referencePrice: 498, openPrice: 499, isin: "US5949181045", description: "Microsoft" },
    { ticker: "AAPL", observedAt: "17:25:00", priceObservedAt: "17:25:00", bookObservedAt: "", metadataObservedAt: "17:24:59", lastPrice: 250, lastQuantity: 10, dayLow: 245, dayHigh: 252, bidPrice: null, bidQuantity: null, askPrice: null, askQuantity: null, referencePrice: 248, openPrice: 249, isin: "US0378331005", description: "Apple" },
    { ticker: "SAP", observedAt: "17:30:01", priceObservedAt: "17:30:01", bookObservedAt: "17:30:01", metadataObservedAt: "17:29:58", lastPrice: 230, lastQuantity: 5, dayLow: 225, dayHigh: 232, bidPrice: 229.9, bidQuantity: 50, askPrice: 230.1, askQuantity: 60, referencePrice: 228, openPrice: 229, isin: "DE0007164600", description: "SAP" },
  ],
  errors: [],
...

{
  ...
  assert.equal(result.freshExecutableBookQuotes, 2);
  assert.equal(result.staleExecutableBookQuotes, 0);
  ...
  assert.equal(msft?.observedAt, "2026-09-21T15:30:02.000Z", "PAPER evidence timestamp must be the BIDASK timestamp");
...
{
  const freshPriceStaleBook = {
    ...snapshot,
    quotes: snapshot.quotes.map((quote) => quote.ticker === "MSFT" ? {
      ...quote,
      observedAt: "17:30:02",
      priceObservedAt: "17:30:02",
      bookObservedAt: "17:20:00",
    } : quote),
  };
  const result = buildDirectaExecutionEvidence(freshPriceStaleBook, instruments, "2026-09-21T15:30:05.000Z", {
    realtimeEntitlementConfirmed: true,
    confirmedRealtimeMarketMics: ["XNAS"],
  });
  assert.equal(result.observations.find((item) => item.symbol === "MSFT")?.eligibility, "VALIDATION_ONLY", "fresh PRICE must not mask stale BIDASK evidence");
  assert.equal(result.staleExecutableBookQuotes, 1);
  assert.ok(result.warnings.some((warning) => warning.includes("BIDASK")));
}

{
  const legacySingleTimestamp = {
    ...snapshot,
    quotes: snapshot.quotes.map((quote) => quote.ticker === "MSFT" ? {
      ...quote,
      bookObservedAt: "",
  ...
  const result = buildDirectaExecutionEvidence(legacySingleTimestamp, instruments, "2026-09-21T15:30:05.000Z", {
    realtimeEntitlementConfirmed: true,
    confirmedRealtimeMarketMics: ["XNAS"],
  });
  assert.equal(result.observations.find((item) => item.symbol === "MSFT")?.eligibility, "VALIDATION_ONLY", "legacy snapshots without explicit BIDASK timestamp must fail closed");
}
