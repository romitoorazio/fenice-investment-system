...
export function parseStooqTimestamp(dateValue, timeValue, timeZone = "Europe/Warsaw") {
...
function validationConfidenceCredit(item) {
  const status = String(item?.status || "").toLowerCase();
  if (status === "confermato") return 1;
  if (status === "divergente") return 0;
  if (status !== "attenzione") return 0;

  // ATTENTION gets graded credit only when the consensus engine explicitly
  // emitted a bounded confidenceCredit. Legacy or hand-crafted ATTENTION rows
  // without that provenance stay fail-closed at zero.
  const explicit = Number(item?.confidenceCredit);
  return Number.isFinite(explicit) ? clamp(explicit, 0, 1) : 0;
}
...
export function computeIntelligenceConfidence({
  ...
  const validationCredit = validations.reduce((sum, item) => sum + validationConfidenceCredit(item), 0);
  const validationCreditRatio = checked > 0 ? validationCredit / checked : 0;
  ...
  const components = {
    ...
    crossValidation: 20 * validationCreditRatio * depth,
  ...
  return {
    ...
    metrics: {
      ...
      attention,
      divergent,
      weightedValidationCredit: Number(validationCredit.toFixed(3)),
      validationCreditPercent: Number((validationCreditRatio * 100).toFixed(1)),
      sourceCount,
      assetClassCount,
