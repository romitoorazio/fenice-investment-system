...
export function buildCalibrationSamplesFromLedger(records, { checkpoint = "7d", decision = "COMPRA" } = {}) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    const confidence = clampProbability(confidenceRaw / 100);
...
export function evaluateCalibration(samples, { bins = 10, minSamples = 30 } = {}) {
  ...
  for (const sample of Array.isArray(samples) ? samples : []) {
    const confidence = clampProbability(sample?.confidence);
    const outcomeRaw = Number(sample?.outcome);
    if (confidence === null || !Number.isFinite(outcomeRaw)) continue;
    valid.push({ confidence, outcome: outcomeRaw >= 0.5 ? 1 : 0 });
  }
