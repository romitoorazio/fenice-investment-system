...
export function executionMatchesPaperFxEvidence(execution, fxEvaluation) {
  ...
  const sameProvider = String(execution.fxProvider || "").trim().toLowerCase()
    === String(fxEvaluation?.metrics?.provider || "").trim().toLowerCase();
  const sameObservedAt = parseTime(execution.fxObservedAt) !== null
    && parseTime(execution.fxObservedAt) === parseTime(fxEvaluation?.metrics?.usdObservedAt);
