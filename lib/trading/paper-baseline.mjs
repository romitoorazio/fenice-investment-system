import { evaluatePaperFxEvidence } from "./paper-fx-evidence.mjs";
...
export function evaluatePaperBaselineEligibility({
  ...
  const fx = evaluatePaperFxEvidence({ fxEvidence, approval, now });
  if (!fx.ready) reasons.push(`PAPER market FX gate is not satisfied: ${fx.reasons.join("; ")}`);
