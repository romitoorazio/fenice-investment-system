...
export function evaluatePaperBaselineEligibility({
  ...
  const directaPilotCoverageReady = directaPilotCandidateSymbols >= minimumDirectaPilotEligibleSymbols
  ...
  if (!broadCoverageReady || !coverageFresh || !coverageMatchesEvidence || !coveragePolicyReady) {
    reasons.push(`per-symbol PAPER execution coverage is insufficient or stale (${paperEligibleSymbols}/${requestedSymbols}, ${paperEligiblePercent}%)`);
  }
  if (!directaPilotCoverageReady) {
    reasons.push(`Directa pilot equity/ETF PAPER coverage is insufficient (${directaPilotEligibleSymbols}/${directaPilotCandidateSymbols})`);
  }
