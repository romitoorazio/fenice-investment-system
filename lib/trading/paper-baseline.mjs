...
export function evaluatePaperBaselineEligibility({
  ...
  const coveragePolicyReady = Number(executionCoverage?.version || 0) >= 3
  ...
  if (!directaPilotCoverageReady) {
    reasons.push(`Directa pilot equity/ETF PAPER coverage is insufficient or lacks broker-backed evidence (${directaPilotEligibleSymbols}/${directaPilotCandidateSymbols})`);
  }
